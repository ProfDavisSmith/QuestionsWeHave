...
\thoughtex{The Movie Theaters}{Consider for a moment the old reel movie projectors, or, for the younger generation, downloading a movie from a less than reputable website. When the movie is playing, there are actually two independent processes going on. First, we have the video being projected onto the screen (or the video file running). There is no audio for this, it is silent. Second, we have the audio recording which is played through speakers (or the audio file which is played). There is no video in this. To fully appreciate the movie, the audio and the video must be played in harmony. It is quite obvious when the video and audio tracks are out of sync (or worse, when the wrong audio is played for the video). But, something happening in the video causing a sound to be heard is actually an illusion. There is no interaction, the tracks are run in parallel.}{movietheater2.jpg}{A movie projector projecting a `western' onto a screen in a theater}   
